...
class PlayerFish(Fish):
    ...
    def __init__(self, game_object, position=None):
        super().__init__(game_object=game_object, texture_map=player_fish_texture_map,
                         size=player_start_size, is_facing_right=True)
        if position is None:
    ...
    def change_movement_direction(self, direction: tuple):
        ...
        self.target_velocity = (
            direction[0] / norm * player_fish_speed, direction[1] / norm * player_fish_speed)
    ...
    def on_update(self: "PlayerFish", delta_time=None):
        ...
        player_acceleration_time_constant = \
            ...
            player_start_size_acceleration_time_constant
        # typically is 0.8 for 60 FPS
        slipperiness = min(
            max(1-delta_time/(player_acceleration_time_constant), 0), 0.97)
        self.velocity = (self.velocity[0]*(slipperiness) + self.target_velocity[0]*(
            1-slipperiness)), (self.velocity[1]*(slipperiness) + self.target_velocity[1]*(1-slipperiness))
        # cycle if out of bounds

        # handle eating
        # check for collisions
        collisions = arcade.check_for_collision_with_list(
            self, self.game_object.fish_sprites)
    ...
    def handle_wall_collision(self):
        if self.width < self.game_object.width:
            if self.position[0] > self.game_object.width - self.width / 2:
                self.position = (self.game_object.width -
                                 self.width / 2, self.position[1])
